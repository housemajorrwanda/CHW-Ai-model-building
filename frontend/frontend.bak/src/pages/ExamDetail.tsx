...
export default function ExamDetail() {
  ...
  const { data: course } = useQuery({
    ...
    queryFn: () => coursesAPI.getById(exam!.courseId),
    ...
    return (
      <Card key={question.id} className={level > 0 ? 'ml-8 border-l-2' : ''}>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Gold Solution Steps */}
          {(question.goldSolution?.steps || question.goldSolutionSteps || []).length > 0 && (
            <div className="border-t pt-4">
              ...
              <div className="space-y-2">
                {(question.goldSolution?.steps || question.goldSolutionSteps || []).map((step: any, idx: number) => (
