...
export default function CreateExam() {
  ...
  const transformApiQuestionToQuestion = (apiQ: any, number: number, parentId?: string): Question => {
    // Handle both API formats: goldSolution.steps or goldSolutionSteps
    const goldSteps = apiQ.goldSolution?.steps || apiQ.goldSolutionSteps || [];
    ...
    return {
      ...
      goldSolutionSteps: goldSteps.map((step: any) => ({
      ...
      finalAnswer: apiQ.goldSolution?.finalAnswer || apiQ.finalAnswer || '',
      finalAnswerLatex: apiQ.goldSolution?.finalAnswerLatex || apiQ.finalAnswerLatex || '',
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={courseId} onValueChange={setCourseId} required disabled={coursesLoading}>
                  <SelectTrigger>
                    <SelectValue placeholder={coursesLoading ? "Loading courses..." : courses.length === 0 ? "No courses available" : "Select a course"} />
                  </SelectTrigger>
                  <SelectContent>
                    {coursesLoading ? (
                      <SelectItem value="loading" disabled>Loading courses...</SelectItem>
                    ) : courses.length === 0 ? (
                      <SelectItem value="none" disabled>No courses available. Create a course first.</SelectItem>
                    ) : (
                      courses.map((course) => (
                ...
                {coursesError && (
                  <p className="text-sm text-destructive">Failed to load courses. Please refresh the page.</p>
                )}
              </div>
            </div>
